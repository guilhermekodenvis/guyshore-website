...
`.claude/launch.json` pins the dev server to **3000** (`npm run dev -- --port 3000`), with `autoPort: false`. The second entry, `guyshore-attach`, starts nothing: it opens the Browser pane on a server already running on 3000, for when another session owns it. `guyshore-dev-3001` starts the same server on 3001, for when 3000 is taken by another project (the Alttavia landing page's dev server has held it).
...
**The header is one element that morphs into a floating island** once the page scrolls past 16px (`src/lib/scroll-detached.ts`, a `useSyncExternalStore` store with hysteresis, read the same way consent is). The `<header>` keeps a fixed 4.5rem box with `pointer-events: none`, so the hero's `100dvh - 4.5rem` still holds and the empty space beside the island does not swallow clicks; `.nav-shell` inside it is what moves. The material is modeled on Apple's Liquid Glass and was chosen by rendering candidate recipes over the real page backgrounds against Apple's own images, in two rounds: the first (a 74% white pill with a drop shadow) was rejected on sight as "about 30% Apple", because Apple's glass is nearly clear, refracts the page along its rim and goes dark over dark content. Three pieces now do that:

- **The material** (`globals.css`, the `.nav-shell` block): clear glass, a 14% paper tint over `blur(4px) saturate(1.2) brightness(1.03)` so the page stays sharp under it (the client called the 11px version "frosted"), one thin 1px ring lit from the top-left and again on the bottom-right cap (`.nav-shell::before`, a masked gradient ring), a small sheen on each cap (`.nav-sheen::before`), a far two-layer float shadow and a faint inset edge density. Every state change runs through opacity (each gradient sits on its own overlay and fades), background-color, box-shadow and backdrop-filter, whose function list keeps the same shape in every state so it interpolates. The mobile menu opens on a 92% paper panel: the clear glass cannot carry a list of links over page content.
- **The tone** (`src/lib/nav-tone.ts`, `data-tone` on the shell): the island reads what is under it and flips material and chrome together. `light` is clear glass with ink labels; `dark`, over an ink surface spanning the island, is smoky glass (9% paper) with paper labels, an inverted mark and a paper Contact pill; `mixed`, when something dark, an image or a heading of 36px or more sits under part of it, is the light glass tinted up to 62% so ink labels stay legible; with a 4px blur a heading would otherwise show sharp under the labels. Full-bleed dark surfaces opt in with `data-nav-tone="dark"` (feasibility panel, footer, contact banner, portfolio band); everything else is hit-tested at six points along the island's center line, placed on the island's settled geometry computed from the header (the only scroll event arrives on the first frame of the morph, when the live box is still the full-width bar) and measured again 600ms later, and a light line drawing such as the hero mascot or a partner logo says so with `data-nav-luma="light"`, otherwise an image counts as unknown. Labels are steel on the attached bar and ink on the detached light island: no tint under 55% white carries steel over the hero heading, and ink is also what Apple puts on clear glass. The Tailwind `group-data-[tone=dark]:` and `group-data-[detached=true]:` variants on the header do the flipping.
- **The lens** (`src/components/nav-lens.tsx`, `data-lens="on"` on the shell): an SVG filter (`#nav-lens`: settle blur, `feDisplacementMap` fed by a canvas-drawn map of the pill, frost, with an alpha clamp after each blur because Chromium ignores `edgeMode` inside backdrop-filter) that `backdrop-filter: url(#nav-lens)` pulls in, bending the page along the rim. Chromium only, gated on the Chromium brand in `navigator.userAgentData` plus a fine pointer: Safari and Firefox parse `url()` as valid and then paint no backdrop at all, so nothing but the driver may ever set the attribute, and phones keep the plain blur, which is the complete material on its own. A `url()` cannot interpolate, so the driver parks the filter at the exact look of `blur(12px)` on the frame the island detaches, sets the attribute, then ramps displacement and blur on the same spring the CSS uses, re-targeting from the current values if the scroll reverses; on the way back it ramps out first and removes the attribute only then. The selector therefore carries no `[data-detached]` and leaves backdrop-filter out of its transition. The map is drawn at the island's settled size computed from the header (not measured mid-morph), cached per width, redrawn only when a settled width changes, never drawn for the open menu.

Four traps, each found the hard way:

- **The material lives on `.nav-glass` and `.nav-sheen`, children, never on the shell.** An element with `backdrop-filter` is the backdrop root for its descendants, so with the blur on the shell the services dropdown sampled the shell's own surface and its blur did nothing (Chrome and Firefox; Safari samples through, which gave three browsers two looks). Never put `opacity`, `filter`, `mask` or `isolation: isolate` on `.nav-shell`: each one makes it a backdrop root again and silently re-breaks the dropdown. `.nav-sheen` must never fade either: its two pseudo-elements carry the two tones' sheens and fade on their own. The reduced-transparency, `prefers-contrast: more` and no-`backdrop-filter` fallbacks are tone-aware (97% paper, or 97% ink under the dark tone, because the chrome is already paper there) and target `.nav-glass` and `.glass-panel`.
...
The two Contact pills carry `focus-visible:outline-ink` (and `outline-paper` under the dark tone): the global `currentColor` ring is white on them, drawn on the light island (1.00:1 on paper, 1.76:1 over an ink section). Same reasoning as `on-ink`, applied the other way round.

**Verifying the header needs headless Chrome, not the app's Browser pane.** The pane stops drawing when its window is hidden, and without frames no scroll event fires and neither store updates, so `data-detached` and `data-tone` read stale. Puppeteer against the dev server (viewport 1440x900 or 375x812, `window.scrollTo` with `behavior: "instant"`, a 500ms wait, then `dataset` and computed styles) is what the checks in this repo's history were made with; `html { scroll-behavior: smooth }` is why the instant behavior matters.
