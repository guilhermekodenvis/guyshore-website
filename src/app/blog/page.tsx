      ...
      <div className="mx-auto max-w-[76rem] px-6 pb-24 lg:px-10 lg:pb-32">
        {posts.length === 0 ? (
          <div className="max-w-[48ch] border-t border-[var(--color-line)] pt-10">
            <p className="text-lead text-tide">
          ...
          <ul>
            {posts.map((post) => (
              <li key={post.slug}>
                <Link
                  href={`/blog/${post.slug}`}
                  className="grid gap-4 border-b border-[var(--color-line)] py-10 transition-colors hover:bg-foam lg:grid-cols-[11rem_1fr] lg:gap-12 lg:py-12"
                >
                  <p className="font-mono text-[0.6875rem] tracking-[0.1em] text-surf uppercase">
                    <time dateTime={post.meta.date}>
                      {formatPostDate(post.meta.date)}
                    </time>
                    <span className="mt-1 block text-tide">
                  ...
                  <div>
                    ...
                    <p className="mt-3 max-w-[60ch] text-tide">
                    ...
                    <p className="mt-5 flex flex-wrap gap-2">
                      {post.meta.tags.map((tag) => (
                        <span
                          key={tag}
                          className="rounded-[2px] border border-[var(--color-line)] px-2.5 py-1 font-mono text-[0.6875rem] text-surf"
                        >
                          {tag}
                        </span>
                      ))}
